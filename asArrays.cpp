...
int main()
{
	...
	{
		std::fill_n(previous_plate[i], PLATE_SIZE, NEUTRAL_TEMP);
	}
		//bottom row get's 100 degrees
	std::fill_n(previous_plate[PLATE_SIZE-1], PLATE_SIZE, 100);

		//mid section (400,0-330) gets 100 degrees
	std::fill_n(previous_plate[400], 330, 100);
	
		//single point gets 100 degrees
	previous_plate[200][500] = 100;
		//Side bars get 0 degrees
	for (int i = 0; i < PLATE_SIZE; ++i)
	{
		...
		//Top bar get's 0
	std::fill_n(previous_plate[0], PLATE_SIZE, 0);


	//Set current_plate to also have the same setup
	std::copy(previous_plate, previous_plate+PLATE_SIZE*PLATE_SIZE, current_plate);
